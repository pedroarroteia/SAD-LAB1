#include <p24fxxxx.h>
#include <stdio.h>
#include <string.h>
#include <math.h>

// Configuration Bits
#ifdef __PIC24FJ64GA004__ //Defined by MPLAB when using 24FJ64GA004 device
_CONFIG1( JTAGEN_OFF & GCP_OFF & GWRP_OFF & COE_OFF & FWDTEN_OFF & ICS_PGx1 & IOL1WAY_ON) 
_CONFIG2( FCKSM_CSDCMD & OSCIOFNC_OFF & POSCMOD_HS & FNOSC_PRI & I2C1SEL_SEC)
#else
_CONFIG1( JTAGEN_OFF & GCP_OFF & GWRP_OFF & COE_OFF & FWDTEN_OFF & ICS_PGx2) 
_CONFIG2( FCKSM_CSDCMD & OSCIOFNC_OFF & POSCMOD_HS & FNOSC_PRI)
#endif

//U2MODE: UARTx Mode Register
//U2RXREG: UARTx Receive Register
//U2STA: UARTx Status and Control Register
//U2TXREG: UARTx Transmit Register (Write-Only) 
//U2BRG: UARTx Baud Rate Generator Register

	int i = 0;
	int independentChannel = 1;
	int actvAlert = 0;
	int ADCValue;
	
	//vetores para guardar strings
	char receivestring[100];
	char str[100];
	
	//vetores para guardar amostras
	int AN0[5];
	int AN4[5];
	int AN5[5];
	int B7[0];
	int D6[0];
	int D7[0];	

	//Limite dos contadores
	int counterTimeoutAN0 = -1;
	int counterTimeoutAN4 = -1;
	int counterTimeoutAN5 = -1;
	int counterTimeoutB7 = -1;
	int counterTimeoutD6 = -1;
	int counterTimeoutD7 = -1;
	
	//contadores
	int counterTimerAN0 = 0;
	int counterTimerAN4 = 0;
	int counterTimerAN5 = 0;
	int counterTimerB7 = 0;
	int counterTimerD6 = 0;
	int counterTimerD7 = 0;
	
	//Limite do numero de amostras
	int counterNumOutAN0 = 1;
	int counterNumOutAN4 = 1;
	int counterNumOutAN5 = 1;
	int counterNumOutB7 = 1;
	int counterNumOutD6 = 1;
	int counterNumOutD7 = 1;	
	
	//numero de amostras
	int counterNumAN0 = 0;
	int counterNumAN4 = 0;
	int counterNumAN5 = 0;
	int counterNumB7 = 0;
	int counterNumD6 = 0;
	int counterNumD7 = 0;

	

//////////////////////////////////////////Delay Function///////////////////////////////////////////
//
	void Delay(unsigned int iter){
		for( i = 0 ; i < iter ; i++){};
	}
//		
//////////////////////////////////////////Initial Configuration Functions/////////////////////////////////
//
	void UART_config(){
		U2BRG = 12; // Baud rate = 19200; FCy=Fosc/2=8M/2=4M; UxBRG =12 
		U2MODE =  0x8000;	//Enable Uart for 8-bit data
		U2STA = 0x0400;		 	
	}	
	
	void ADC_config(){
		AD1PCFG = 0xFFCA; // AN0 AN2 AN4 and AN5 as analog, all other pins are digital
		AD1CON1 = 0x0000; // SAMP bit = 0 ends sampling and starts converting
		AD1CHS = 0x0005; // Connect AN5 as S/H+ input
		AD1CSSL = 0;
		AD1CON3 = 0x0002; // Manual Sample, Tad = 3Tcy
		AD1CON2 = 0;
		AD1CON1bits.ADON = 1; // turn ADC ON
	}

	void button_config(){	
		TRISDbits.TRISD6 = 1; //button 6 (1 means input)//active low
		TRISDbits.TRISD7 = 1; //button 7 (1 means input)//active low	
	}
	
	void led_config(){	
		TRISAbits.TRISA0 = 0; //Led 0 (0 means output)
		TRISAbits.TRISA1 = 0; //Led 1 (0 means output)
		TRISAbits.TRISA4 = 0; //Led 4 (0 means output)
	}
	void start_timer(){
		T1CON = 0x00; //Stops the Timer1 and reset control reg.
		TMR1 = 0x00; //Clear contents of the timer register
		PR1 = 0x0FA0; //Load the Period register with the value 0x0FA0
		IPC0bits.T1IP = 0x01; //Setup Timer1 interrupt for desired priority level
		IFS0bits.T1IF = 0; //Clear the Timer1 interrupt status flag
		IEC0bits.T1IE = 1; //Enable Timer1 interrupts
		T1CONbits.TON = 1; //Start Timer1 with prescaler settings at 1:1 and 
	}
	
	void configuration(){
					  
		ADC_config();
		UART_config();
		button_config();
		led_config();
		TRISAbits.TRISA7 = 0; //Bidirecional (0 - output, 1 - input)		
		Delay(2);
		start_timer();
	
	}
//
//////////////////////////////////////////Transmition Functions///////////////////////////////////////////
//
	void TransmitChar(char data){
		while(U2STAbits.UTXBF == 1){} //while transmiter buffer is full. wait
		U2TXREG = data;  //transmitir data para o resgisto de transmisão	
	}
	
	void TransmitString(char* string){
		for( i=0;string[i]!='\0';i++){ //Envia uma string caracter a caracter
			TransmitChar(string[i]);  //transmitir string 
		}
	}

//	
//////////////////////////////////////////Analog Channels Functions///////////////////////////////////////////	
//
	int ADC_read(int channel){	
		AD1CHS = channel; // Connect AN5 as S/H+ input
		AD1CON1bits.SAMP = 1; // start sampling
		Delay(1);
		AD1CON1bits.SAMP = 0; // start converting
		while (!AD1CON1bits.DONE){}; // wait for the conversion to be done		
		ADCValue = ADC1BUF0;
		return ADCValue;
	}	
	
	void transmitADC(int channel, int counterNumAN, int* AN_val){
	    sprintf(str,"{\"An%d\":[",channel);
	    TransmitString(str);
		int j = 0;
		
		while(1){				
			if(j ==(counterNumAN-1)){
				sprintf(str,"%d]}\n\r",AN_val[j]);
	        	TransmitString(str);
				break;
			}
			else{
	             sprintf(str,"%d, ",AN_val[j]);
	             TransmitString(str);
			}
			j++;
		}	
	}
	void sampleConfig_rate(int sampleRate, int channel){
		
				if (channel == 0){
					counterTimeoutAN0 = sampleRate;
					counterTimerAN0 = 0;
				
				}
				
				if (channel == 4){
					counterTimeoutAN4 = sampleRate;
					counterTimerAN4 = 0;
				}
				
				if (channel == 5){
					counterTimeoutAN5 = sampleRate;
					counterTimerAN5 = 0;
				}
				
	}
	
	void sampleDesactivate(int channel){
		
				if (channel == 0){
					counterTimeoutAN0 = -1;
				}
		
				if (channel == 4){
					counterTimeoutAN4 = -1;
				}
				
				if (channel == 5){
					counterTimeoutAN5 = -1;
				}
				
	}

	void sampleConfig_number(int num, int channel){
	
			if (channel == 0){
				counterNumOutAN0 = num;
				counterNumAN0 = 0;
			
			}
			
			if (channel == 4){
				counterNumOutAN4 = num;
				counterNumAN4 = 0;
			}
			
			if (channel == 5){
				counterNumOutAN5 = num;
				counterNumAN5 = 0;
			}
			
}
//
//////////////////////////////////////////Digital Channels Functions///////////////////////////////////////////
//	
	void transmitDigital(int channel, int counterNumD, int* D_val){
		sprintf(str,"{\"D%d\":[",channel);
	    TransmitString(str);
		int j = 0;
		
		while(1){				
			if(j ==(counterNumD-1)){
				sprintf(str,"%d]}\n\r",D_val[j]);
	        	TransmitString(str);
				break;
			}
			else{
	             sprintf(str,"%d, ",D_val[j]);
	             TransmitString(str);
			}
			j++;
		}
	
	}
	



	void sampleConfig_rate_digital(int sampleRate, int channel){
	
			if (channel == 1){
				counterTimeoutB7 = sampleRate;
				counterTimerB7 = 0;
			}
			if (channel == 6){
				counterTimeoutD6 = sampleRate;
				counterTimerD6 = 0;
			}
			if (channel == 7){
				counterTimeoutD7 = sampleRate;
				counterTimerD7 = 0;
			}
			
			
	}

	void sampleDesactivate_digital(int channel){
	
			if (channel == 7){
				counterTimeoutB7 = -1;
			}
			if (channel == 6){
				counterTimeoutD6 = -1;
			}
			if (channel == 7){
				counterTimeoutD7 = -1;
			}
	}

	void sampleConfig_number_digital(int num, int channel){
	
			if (channel == 1){
				counterNumOutB7 = num;
				counterNumB7 = 0;
			
			}
			if (channel == 6){
				counterNumOutD6 = num;
				counterNumD6 = 0;
			
			}
			if (channel == 7){
				counterNumOutD7 = num;
				counterNumD7 = 0;
			
			}
			
	}
	void config_digital_io(int channel, int io_dir){
		
		if(channel == 1)
			TRISAbits.TRISA7 = io_dir; //(0 - output, 1 - input)
	}
	
	int buttonRead(){
			if(independentChannel == 0)
				return (PORTDbits.RD7* pow(2,1))+(PORTDbits.RD6* pow(2,0));
			else 
				return -1;
	}
//
/////////////////////////////////////////////////////////Led functions	////////////////////////
//
	void led_enable(int led){
		if(led == 0)
			PORTAbits.RA0 = 1;
		if(led == 1)
			PORTAbits.RA1 = 1;
		if(led == 4)
			PORTAbits.RA4 = 1;
	}
	
	void led_disable(int led){
		if(led == 0)
			PORTAbits.RA0 = 0;
		if(led == 1)
			PORTAbits.RA1 = 0;
		if(led == 4)
			PORTAbits.RA4 = 0;
	}


	

//
///////////////////////////////////////Atribute functions////////////////////////////////
//
	void samplingPeriodCounter(int channel, int vector[], int counterTimer,int counterTimeout, int counterNum, int counterNumOut ){
		if(counterTimeout - 1 == counterTimer){
			 
			counterTimer = 0;
			vector[counterNum++] = ADC_read(channel);
					
			if(counterNumOut == counterNum){
				transmitADC(channel, counterNum, vector);
				counterNum = 0;
			}	
		}
			counterTimer++;
	}

	void samplingPeriodCounterDigital(int channel, int vector[],int value, int counterTimer,int counterTimeout, int counterNum, int counterNumOut ){
		if(counterTimeout - 1 == counterTimer){
				counterTimer = 0;
				vector[counterNum++] = value;

				if(counterNumOut == counterNum){
	        	
					transmitDigital(channel, counterNum, vector);
					counterNum = 0;
				}
		}
		counterTimer++;
	}
//	
	void  __attribute__((__interrupt__, __shadow__)) _T1Interrupt(void){	
		
		samplingPeriodCounter(0, AN0, counterTimerAN0,counterTimerAN0, counterNumAN0, counterNumOutAN0 );
		samplingPeriodCounter(4, AN4, counterTimerAN4,counterTimerAN4, counterNumAN4, counterNumOutAN4 );
		samplingPeriodCounter(5, AN5, counterTimerAN5,counterTimerAN5, counterNumAN5, counterNumOutAN5 );
	    /*
		if(counterTimeoutAN0 - 1 == counterTimerAN0){
					
			if(counterNumOutAN0 == counterNumAN0){
				transmitADC(0, counterNumAN0, AN0);
				counterNumAN0 = 0;
			}	
		}

		if(counterTimeoutAN4 - 1 == counterTimerAN4){
			counterTimerAN4 = 0;
			AN4[counterNumAN4++] = ADC_read(4);
			
			if(counterNumOutAN4 == counterNumAN4){
	        	
				transmitADC(4, counterNumAN4, AN4);
				counterNumAN4 = 0;
			}
		}
		if(counterTimeoutAN5 - 1 == counterTimerAN5){
			counterTimerAN5 = 0;
			AN5[counterNumAN5++] = ADC_read(5);

			if(counterNumOutAN5 == counterNumAN5){
	        	
				transmitADC(5, counterNumAN5, AN5);
				counterNumAN5 = 0;
			}
		}		
		*/
/////////////////////////////////////////
	samplingPeriodCounterDigital(1, B7,PORTAbits.RA7   , counterTimerB7, counterTimeoutB7, counterNumB7, counterNumOutB7 );
	samplingPeriodCounterDigital(6, D6,TRISDbits.TRISD6, counterTimerD6, counterTimeoutD6, counterNumD6, counterNumOutD6 );
	samplingPeriodCounterDigital(7, D7,TRISDbits.TRISD7, counterTimerD7, counterTimeoutD7, counterNumD7, counterNumOutD7 );
	/*
		if (TRISAbits.TRISA7 == 0) { //se etiver em modo output
			if(PORTAbits.RA7 == 0)
				PORTAbits.RA7 = 1;
			else
				PORTAbits.RA7 = 0;
		}
		else{
			 if(counterTimeoutB7 - 1 == counterTimerB7){
				counterTimerB7 = 0;
				B7[counterNumB7++] = PORTAbits.RA7;
	
				if(counterNumOutB7 == counterNumB7){
	        	
					transmitDigital(7, counterNumB7, B7);
					counterNumB7 = 0;
				}
			}
		}	
		////
		if(counterTimeoutD6 - 1 == counterTimerD6){
			counterTimerD6 = 0;
			D6[counterNumD6++] = PORTDbits.RD6;
			
			if(counterNumOutD6 == counterNumD6){
        	
				transmitDigital(6, counterNumD6, D6);
				counterNumD6 = 0;
			}
		}
		
		if(counterTimeoutD7 - 1 == counterTimerD7){
			counterTimerD7 = 0;
			D7[counterNumD7++] = PORTDbits.RD7;

			if(counterNumOutD7 == counterNumD7){
        	
				transmitDigital(7, counterNumD7, D7);
				counterNumD7 = 0;
			}
		}
		
		counterTimerAN0++;
		counterTimerAN4++;
		counterTimerAN5++;
		counterTimerB7++;
		counterTimerD6++;
		counterTimerD7++;
			*/
		if(actvAlert == buttonRead())
			TransmitString("Alerta - Detecao no Canal Virtual ");
			

		IFS0bits.T1IF = 0; //Reset Timer1 interrupt flag and Return from ISR

	}


//	
//////////////////////////////////////////Receive Functions///////////////////////////////////////////
//
	char ReceiveChar(){
		while(U2STAbits.URXDA == 0){} //while receiver buffer is empty. wait.
		return U2RXREG; //receber data do resgisto de receção
	}
	
			
	int convert_string_int(char * str, int lenArg) {
		int sum = 0;
		int n = 0;
		int i = 0;
		int number;
		
		for (n = lenArg - 1, i = 0; n >= 0; n--, i++) {			
			number = ((int)str[i] - 48);
			sum = sum + number *pow(10,n);	
		}
			
		return sum;
	}
//
	void ReceiveString(){

		receivestring[0]=ReceiveChar();		
		i=1;
		int n = 0;
		char json[20];
		char argJson_aux[20];
		int argJson[20];
		int controlJson1 = 0;
		int controlJson2 = 0;
		int controlJson3 = 0;
		int controlJson4 = 0;
		int controlJson5 = 0;

		while(1){ //Envia uma string caracter a caracter
			receivestring[i] = ReceiveChar();  //transmitir string 
			if(receivestring[i]=='}'){
				break;
			}
			i++;
		}
		receivestring[i+2]= '\n';
		receivestring[i+2]= '\0';


		//ack
		TransmitString(receivestring);	
			
		while(1){ 

			if('"' == receivestring[n]){
				controlJson3++;
				controlJson1 = 0;
			}
			else if('[' ==  receivestring[n]){
				controlJson4 = 1;
				controlJson2 = 0;
				controlJson5 = 0;
			}
			else if(']' ==  receivestring[n]){
				controlJson4 = 0;
				//TransmitString(json);
			
				// fiz uma alteração aqui se não funcionar vê a versão anterior
				if((strcmp(json,"analogAmostragem")==0) && (argJson[1] == 1)){
					sampleConfig_rate(1000, argJson[0]);
				}
				else if((strcmp(json,"analogAmostragem")==0) && (argJson[1] == 0)){
					sampleDesactivate(argJson[0]);
				}
				//
				
				else if(strcmp(json,"freqAmostragem")==0){
					sampleConfig_rate(argJson[1], argJson[0]);
				}
				else if(strcmp(json,"numAmostragem")==0){
					sampleConfig_number(argJson[1], argJson[0]);
				}
				else if(strcmp(json,"Bidirecional")==0){
					config_digital_io(argJson[0], argJson[1]);
				}
				
				// fiz uma alteração aqui se não funcionar vê a versão anterior
				else if((strcmp(json,"DigitalAmostragem")==0) && (argJson[1] == 1) ){
					sampleConfig_rate_digital(1000, argJson[0]);
				}
				else if((strcmp(json,"DigitalAmostragem")==0)  && (argJson[1] == 0)){
					sampleDesactivate_digital(argJson[0]);
				}
				//
				
				else if(strcmp(json,"DigitalFreq")==0){
					sampleConfig_rate(argJson[1], argJson[0]);
				}
				else if(strcmp(json,"DigitalNum")==0){
					sampleConfig_number_digital(argJson[1], argJson[0]);
				}
				else if(strcmp(json,"virtual")==0){
					independentChannel = argJson[0];
				}
				else if(strcmp(json,"alertEvent")==0){			
					actvAlert = argJson[n];
				}
				
				// fiz uma alteração aqui se não funcionar vê a versão anterior
				else if((strcmp(json,"Led")==0) && (argJson[1] == 1)){
						led_enable(argJson[0]);
				}
				else if((strcmp(json,"Led")==0) && (argJson[1] == 0)){
					led_disable(argJson[0]);
				}				
				else if((strcmp(json,"BidirecionalActive")==0) && (TRISAbits.TRISA7 == 0)){  //se etiver em modo output
					PORTAbits.RA7 = 1;
					Delay(1000);
					PORTAbits.RA7 = 0;
					
				}
				//
			}
			else if ((' ' == receivestring[n]) || (':' == receivestring[n]) || (',' == receivestring[n] && controlJson4 == 0)){} //não faz nada
			else{
	
				if( (controlJson3%2) == 1  ){
					json[controlJson1++] = receivestring[n];
					json[controlJson1] = '\0';
				}
				else if(controlJson4 == 1){
					if(receivestring[n] != ',' )
						argJson_aux[controlJson2++] = receivestring[n];
					else {
						argJson[controlJson5++] = convert_string_int(argJson_aux, controlJson2);
						controlJson2 = 0;
					}
					if(receivestring[n + 1] == ']'){
						argJson[controlJson5++] = convert_string_int(argJson_aux, controlJson2);
						controlJson2 = 0;
					}
				}
			}
			
			
			// Fechar o intrepertador
			if(receivestring[n] == '}'){
				break;
			}
			n++;			
		}			
	}

//	
//////////////////////////////////////////MAIN///////////////////////////////////////////
//
	int main(void){
		
		configuration();
			
		while ( 1 ){
				//sprintf(str,"UVVAA SECA");
	            //TransmitString(str);
				ReceiveString();
				
			//botão para ativar as duas saidas digitais
			if ( (!PORTDbits.RD6) && (independentChannel == 1)){		
				Delay(100);
				PORTAbits.RA1 = 1;
				Delay(100);
				PORTAbits.RA0 = 1;
				Delay(20000);
				PORTAbits.RA1 = 0;
				Delay(100);
				PORTAbits.RA0 = 0;
				Delay(20000);
			
			}
			if ( (!PORTDbits.RD7) && (independentChannel == 1)){
				Delay(100);
				PORTAbits.RA4 = 1;
				Delay(20000);
				PORTAbits.RA4 = 0;
				Delay(20000);
			}
		}
	}
